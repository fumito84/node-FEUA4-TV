fetch('http://localhost:3000/') // istraukiami suvesti duomenys ir atvaiduoti kaip lista
.then((resp) => resp.json()) // istrauktus duomenis paverciame i JSON formata
.then((response) => {
    const productList = document.querySelector('#products');// response istraukia UL produktu lista kuris bus suvestas  - [juice ,bread].forEach(product => {})

    response.forEach((product) => {
        const li = document.createElement("li"); //sukuriamas elementas kai vedamas naujas produktas
        li.textContent = product; // sukurtam elementui priskiriamas tekstas
...
//body - musu siunciami duomenys reikia prideti JSON.stringify(data tam kad serveris suprastu siunciamus duomens), butinai turi buti JSON formatu, arba JAVASCRIPT object pvs: {name: "Tomas", surname: "Vasiljevas"}
    ...
    fetch("http://localhost:3000/", { 
        ...
        body: JSON.stringify({product}), // pagrindine duomenu dalis su papildoma stringify funkcija kad beckend suprastu kokie duomenys pateikiami - string 
    })
        .then((resp) => resp.json())
        .then((responce) => {
